...
try:  # pragma: no cover - exercised in integration runtime
    ...
    class HTTPException(Exception):  # noqa: N818 - mimic FastAPI exception type
        def __init__(self, status_code: int, detail: str) -> None:
            self.status_code = status_code
            self.detail = detail
            super().__init__(detail)

    class BackgroundTasks:  # type: ignore[override]
        def add_task(self, func, *args, **kwargs):
            func(*args, **kwargs)

    class Request:  # Minimal stub to satisfy type hints
        async def body(self) -> bytes:  # pragma: no cover - stub
            raise RuntimeError("FastAPI is required to process HTTP requests")
...
class WebhookHandler:
    ...
    def __init__(
        ...
        github_api_factory: Callable[[str], GitHubAPI] | None = None,
    ) -> None:
    ...
    def _default_factory(self, token: str) -> GitHubAPI:
        return GitHubAPI(token, base_url=self.settings.github_api_url)

    async def handle(self, request: Request, background: BackgroundTasks) -> dict[str, str]:
