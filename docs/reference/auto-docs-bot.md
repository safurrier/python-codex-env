...
2. **Context gathering** – The worker fetches pull request metadata and changed files,
   summarising code versus documentation touches so the prompts stay tightly scoped.
...
- **GitHub integration** – `GitHubApp` exchanges installation tokens with JWTs generated
  from the app's private key, while `GitHubAPI` wraps reactions, comments, refs, contents,
  and pull requests using [`githubkit`](https://github.com/yanyongyu/githubkit).
- **Agent runner** – `AgentRunner` coordinates the selected agent strategy, computes the
  light-weight file summaries passed to BAML, validates Markdown-only patches, and handles
  PR authoring so strategies stay git-agnostic.
