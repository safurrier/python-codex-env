...
3. The bot reacts with 👀, delegates to a pluggable agent strategy to explore the
   repository and propose documentation updates, and pushes a new branch prefixed with
   `auto-docs/`.
...
`GITHUB_PRIVATE_KEY`, `CLAUDE_API_KEY`, and `BAML_ENV`. The agent backend can be swapped by
setting `AUTO_DOCS_AGENT_BACKEND` to `claude` (default) or `codex`; when using Codex you can
